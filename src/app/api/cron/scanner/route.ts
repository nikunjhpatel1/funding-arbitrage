...
async function performFetch(budgetMs: number): Promise<ApiResponse> {
  ...
  // Use 90% of the full budget as the deadline.
  // The old hard-cap of 7 s was meant for Vercel Hobby's 10 s limit — don't apply it in dev
  // (budgetMs = 28 000), which was killing Gate.io / BitMEX / Phemex / dYdX / Hyperliquid.
  const deadlineMs = Math.floor(budgetMs * 0.9);
...
export async function GET() {
  ...
  try {
    ...
    const { error: cacheError } = await supabase
      ...
      .upsert({
        id: 1,
        payload: oldGlobalCache,
        updated_at: Date.now(),
      });

    if (cacheError) {
      console.error('[Cron Scanner] Failed to write scanner_cache:', cacheError);
    }
  } finally {
    isFetching = false;
  }
...
async function saveHistoricalData(entries: FundingRateEntry[]) {
  try {
    ...
    // Insert into Supabase in batches of 500 to prevent payload limits
    const batchSize = 500;
    let insertedCount = 0;
    
    for (let i = 0; i < rows.length; i += batchSize) {
      const batch = rows.slice(i, i + batchSize);
      const { error } = await supabase.from('funding_rate_history').insert(batch);
      if (error) throw error;
      insertedCount += batch.length;
    }

    console.log(`[Supabase] Saved ${insertedCount} rows to funding_rate_history`);
  } catch (e) {
    console.error('[Supabase] Failed to save historical data', e);
  }
}
