...
const DEMO_EXCHANGES = [
  { id: 'bybit', baseName: 'Bybit Demo Trading', color: '#F7A600' },
  { id: 'okx', baseName: 'OKX Demo Trading', color: '#000000' },
  { id: 'kucoin', baseName: 'KuCoin Demo Trading', color: '#24AE8F' },
];
...
export default function DemoSettingsPage() {
  ...
  async function fetchKeys() {
    setLoading(true);
    try {
      const res = await fetch(`/api/settings/demo-keys`);
  ...
  async function handleSave(exchange: string) {
    ...
    try {
      const res = await fetch('/api/settings/demo-keys', {
        ...
        body: JSON.stringify({ exchange, apiKey: form.apiKey.trim(), secret: form.secret.trim() }),
      });
      const data = await res.json();
      if (!data.success) throw new Error(data.error);
      updateForm(exchange, { saving: false, success: 'Testnet keys saved successfully!' });
      fetchKeys();
    } catch (e: any) {
  ...
  async function handleDelete(exchange: string) {
    if (!confirm(`Remove Testnet API keys for ${exchange}?`)) return;
    setDeleting(exchange);
    try {
      await fetch('/api/settings/demo-keys', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleTest(exchange: string) {
    ...
    try {
      const res = await fetch('/api/settings/demo-keys/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ exchange }),
      });
      const data = await res.json();
      if (data.success) {
        setTestStatus(prev => ({ ...prev, [exchange]: { testing: false, result: 'connected', message: '🟢 Connected to Testnet' } }));
      } else {
        ...
        <div style={{ marginBottom: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
            ...
            <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>
              Demo Trading APIs (Real Prices, Fake Money)
            </h1>
          </div>
          <p style={{ color: 'var(--text-secondary)', margin: 0 }}>
            <span style={{ color: '#22c55e', fontWeight: 'bold' }}>SAFE MODE: </span>
            These keys are strictly used against each exchange's official Demo Trading environment (real market prices, virtual funds). 
            Do NOT enter your real-money API keys here.
          </p>
        </div>
